...
import { ArrowLeft, Download, CheckCircle2, Building2, User, CreditCard, Calendar, Hash } from "lucide-react";
...
export default function Receipt() {
  ...
  useEffect(() => {
    ...
    const fetchReceipt = async () => {
      try {
        const data = await userPaymentApi.getHistory(token);
        const transactions = data.transactions || [];
        const found = transactions.find(
          (t) => String(t.id) === String(paymentId) || String(t.paymentRequestId) === String(paymentId)
        );
        if (found) {
          setReceipt(found);
        } else {
  ...
  const handleDownload = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
      <head>
        <title>Payment Receipt - ${receiptData.eventName || "Unknown"}</title>
        <style>
      ...
      <body>
        <div class="header">
          ...
          <p>${receiptData.eventName || "Payment Transaction"}</p>
        </div>
        ...
        <div class="details">
          <div class="detail-row">
            ...
            <span class="detail-value">${receiptData.eventName || "N/A"}</span>
          </div>
          <div class="detail-row">
          ...
          <div class="detail-row">
            ...
            <span class="detail-value" style="color: #22c55e;">${receiptData.status || "Success"}</span>
          </div>
          <div class="detail-row">
    ...
    <div className="min-h-screen bg-cover bg-center bg-no-repeat" style={{ backgroundImage: 'url("/orangegrid.jpg")' }}>
      <div className="min-h-screen bg-orange-50/50">
        ...
        <div className="max-w-2xl mx-auto px-6 py-8">
          {isSuccessful && (
            <div className="bg-gradient-to-r from-green-500 to-green-400 rounded-2xl p-6 mb-6 shadow-lg">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="bg-gradient-to-br from-orange-500 to-orange-300 px-6 py-5">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold text-white/80 uppercase tracking-wide">Payment Receipt</p>
                  <h1 className="text-2xl font-bold text-white mt-1">{receipt.eventName || "Payment"}</h1>
                </div>
                <button
              ...
              <div className="space-y-6">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-900 mt-0.5">
                          ...
                          {receipt.student.roll_no && (
                            <span className="text-gray-500 ml-2">({receipt.student.roll_no})</span>
                          )}
                        </p>
                      </div>
                    </div>
                ...
                {receipt.eventDescription && (
                  <div className="bg-orange-50 rounded-xl p-4">
                    <p className="text-xs text-orange-800 font-medium mb-1">About this payment</p>
                    <p className="text-sm text-orange-900">{receipt.eventDescription}</p>
                  </div>
                )}
              </div>
